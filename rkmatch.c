...
{	
    ...
    for (index; index < len; index++) {
	   	if (isupper(buf[index])) {
    		// Check for uppercase
    		buf[new_position++] = tolower(buf[index]);
    	} else if (isspace(buf[index])) {
    		// First handle the space, insert if space is not at beginning or end
    		// Also remove consecutive space
    		if ((index > 0) && (index < (len-1)) && !isspace(buf[index-1])) {
    			buf[new_position++] = ' ';
    		}
    	} else {
    		// Character passes normalization test
    		buf[new_position++] = buf[index];
    	}
    }
        ...
        const unsigned char *ts, int n)
{
	...
	int i;
	// Iterate through every char in  qs and match
	for (i=0; qs[i]; i++) {
		if (qs[i] != ts[i]) {
			return 0;
		}
						 ...
						 int n						/* the length of the document Y */)
{
	...
	for (i = 0; i < n; i++) 
	{
		// Iteratively search ps (size of k) in ts;
								 ...
								 int n						/* the length of the document Y */ )
{
	...
	int response = 0;
	long long hash = 1;
	long long ps_hasher = 0;
	long long ts_hasher = 0;

	
	// We calculate base^(k-1)
	for (i = 0; i < k - 1; i++) {
		hash = mmul(256, hash);
	}

	// Create initial ps and ts hashes
	for (i = 0; i < k; i++) {
		ps_hasher = madd(mmul(256, ps_hasher), ps[i]);
		ts_hasher = madd(mmul(256, ts_hasher), ts[i]);
	} 
	...
	for (i=0; i < n - k; i++) {
		// Print the hashes
		if (print_counter < PRINT_RK_HASH) {
			printf("%llu ", ts_hasher);
			print_counter++;
		} else if (print_counter == PRINT_RK_HASH) {
			printf("\n");
			print_counter++;
		}

		// Collision check
		if (ps_hasher == ts_hasher) {
			for (j = 0; j < k; j++) {
				if (ts[i + j] != ps[j]) {
					break;
				}
			...
			// Remove last digit
			ts_hasher = mmul(256, mdel(ts_hasher, mmul(ts[i], hash)));
			// Add first digit
			ts_hasher = madd(ts_hasher, ts[i + k]);
			// Make positive
			while (ts_hasher < 0) {
				ts_hasher = ts_hasher + BIG_PRIME;
			}
		}
	}
	return response;
